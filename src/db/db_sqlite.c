...
static DBBackend *sqlite_open(const Config *cfg) {
    ...
    static const char *seed2 =
        ...
        " Discounts: Include discount amount if shown (AKTION, RABATT, etc). null if none."
        " is_receipt: true if valid receipt, false if not."
        " image_quality: 0-100 percent (100=perfect OCR, 0=unreadable)."
        " reported_item_count: number stated on receipt, or null."
        " Other Info: Group remaining data into the other object.'"
        " || '"
        " JSON Schema: {is_receipt,image_quality:0-100,"
        " store_information:{name,address},"
        " line_items:[{id,original_name,english_translation,category,sub_category,"
        " price,discount,tax_group,amount,unit_of_measure}],"
        " total_sum,reported_item_count,"
...
static int sqlite_mark_ocr_done(DBBackend *backend, int64_t id, const char *ocr_file_name) {
    ...
    const char *sql = "UPDATE files SET is_ocr_processed=1, ocr_file_name=?, updated_at=?"
                      " WHERE id=?";
...
static int sqlite_get_parsed_receipt(DBBackend *backend, int64_t file_id, ParsedReceipt *out) {
    ...
    const char *sql = "SELECT id, file_id, parsed_json, created_at, updated_at"
