...
static char *tg_get_file_path(const TgBot *bot, const char *file_id)
{
    ...
    snprintf(url, sizeof(url), "%s%s/getFile?file_id=%s",
             TG_API_BASE, bot->cfg->telegram_token, encoded_id);
    free(encoded_id);
...
static void list_cb(const FileRecord *rec, void *ud)
{
    ...
    if (c->count >= MAX_LIST_ENTRIES) return;

    int remaining = c->cap - c->pos;
    if (remaining <= 0) return;

    int written = snprintf(c->buf + c->pos, (size_t)remaining,
        "%d. %s\n   %s | %lld bytes | OCR: %s\n\n",
        c->count + 1,
        rec->saved_file_name,
        rec->original_file_name,
        (long long)rec->file_size_bytes,
        rec->is_ocr_processed ? "done" : "pending");
...
static int str_starts_with(const char *text, const char *cmd)
{
    ...
    return strncmp(text, cmd, cmd_len) == 0 &&
           (text[cmd_len] == '\0' || text[cmd_len] == ' ');
}
...
static void dispatch_update(TgBot *bot, cJSON *update)
{
    ...
    if (doc) {
        ...
        if (cJSON_IsNumber(fsize) &&
            fsize->valuedouble > MAX_FILE_MB * 1024.0 * 1024.0) {
