...
static inline int db_backend_find_by_hash(DBBackend *db, const char *hash, FileRecord *out) {
    if (!db) {
        LOG_ERROR("db_backend_find_by_hash: db is NULL");
        return -1;
    }
    if (!db->ops) {
        LOG_ERROR("db_backend_find_by_hash: db->ops is NULL");
        return -1;
    }
    if (!db->ops->find_by_hash) {
        LOG_ERROR("db_backend_find_by_hash: db->ops->find_by_hash is NULL");
        return -1;
    }
...
static inline int db_backend_mark_parsing_done(DBBackend *db, int64_t file_id,
...
static inline int db_backend_get_parsed_receipt(DBBackend *db, int64_t file_id,
                                                ParsedReceipt *out) {
